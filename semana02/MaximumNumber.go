class Solution {
	...
	class Solution {
		public:
			// Retorna maior subsequência de tamanho k
			vector<int> getMaxSubsequence(vector<int>& nums, int k) {
				vector<int> stack;
				int drop = nums.size() - k;
				for (int num : nums) {
			...
			// Compara se seq1 > seq2 a partir dos índices
			bool greater(vector<int>& seq1, int i, vector<int>& seq2, int j) {
				while (i < seq1.size() && j < seq2.size()) {
					if (seq1[i] > seq2[j]) return true;
					if (seq1[i] < seq2[j]) return false;
			...
			// Intercala duas sequências gerando o maior número
			vector<int> merge(vector<int>& seq1, vector<int>& seq2) {
				vector<int> merged;
				int i = 0, j = 0;
				while (i < seq1.size() || j < seq2.size()) {
			...
			// Gera o maior número possível com k dígitos
			vector<int> maxNumber(vector<int>& nums1, vector<int>& nums2, int k) {
				vector<int> best;
				int m = nums1.size(), n = nums2.size();
				for (int i = max(0, k - n); i <= min(k, m); ++i) {
	...
	// Complexidade de tempo: O(k * (m + n)^2)
	// Complexidade de espaço: O(k)
	// Existe solução mais simples: Sim, usando pilha
	// Estruturas utilizadas: arrays. Pode ser utilizada pilha
